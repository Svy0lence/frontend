...
export class MatEditarUsuariosComponent implements OnInit {
  ...
  constructor(private api: ApiService, private dialogRef: MatDialogRef<MatEditarUsuariosComponent>,
  ...
  async MostrarRoles() {
    try{
      ...
      for (let i = 0; i < this.rolList.length; i++) {
        if (this.rolList[i].id === this.data.id_rol) {
  ...
  actualizar() {
    if (this.registerForm.valid) {
      ...
      formData.append('estado', this.usuario.estado); // true
